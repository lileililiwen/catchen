...
- Manual: the HTTP smoke tests you exercised (happy path AND negative cases), per Agents.md §4
-->

## Quality-gate checklist

- [ ] OpenSpec change exists with all four artifacts (proposal / spec / design / tasks) complete and validated
- [ ] `dotnet format --verify-no-changes` passes (and `dart format` for client code)
- [ ] Build is clean: 0 warnings / 0 errors under `/warnaserror`
- [ ] Tests pass (backend xUnit + architecture tests; `flutter analyze` + `flutter test`)
- [ ] Spec scenarios verified end-to-end (happy path and negative cases)
- [ ] No stubs, `TODO`s, or `NotImplementedException`
- [ ] Server-side validation and authorization on every mutating endpoint; ownership filters intact
- [ ] Offshore compliance preserved: no domestic payment/promotion channels, no `+86` acceptance, no hardcoded provider keys
