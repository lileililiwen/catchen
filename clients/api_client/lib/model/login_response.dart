...
class LoginResponse {
  ...
  int get hashCode =>
      ...
      (expiresAtUtc == null ? 0 : expiresAtUtc!.hashCode);

  @override
  String toString() =>
      'LoginResponse[token=$token, expiresAtUtc=$expiresAtUtc]';
  ...
  Map<String, dynamic> toJson() {
    ...
    if (this.expiresAtUtc != null) {
      json[r'expiresAtUtc'] = this.expiresAtUtc!.toUtc().toIso8601String();
    } else {
      json[r'expiresAtUtc'] = null;
    }
    return json;
  }
  ...
  static LoginResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "LoginResponse[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "LoginResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return LoginResponse(
        ...
        expiresAtUtc: mapDateTime(json, r'expiresAtUtc', r''),
      );
    }
    return null;
  }

  static List<LoginResponse> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<LoginResponse>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = LoginResponse.listFromJson(
          entry.value,
          growable: growable,
        );
      }
